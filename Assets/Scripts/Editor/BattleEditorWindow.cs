...
public static class BasicTargetEffectAssetGenerator
{
    private const float FireAnimationDuration = 0.5f;
    private const float FireRotationDuration = 0.08f;

    private const string MenuPath =
        "Tools/Dungeon/Effects/Create Basic Target Effect";
    private const string EffectFolder =
        "Assets/Animations/Battle/BasicTarget";
    private const string HiddenClipPath =
        EffectFolder + "/BasicTargetHidden.anim";
    private const string AimClipPath =
        EffectFolder + "/BasicTargetAimIn.anim";
    private const string HoldClipPath =
        EffectFolder + "/BasicTargetAimHold.anim";
    private const string FireClipPath =
        EffectFolder + "/BasicTargetFire.anim";
    private const string ControllerPath =
        EffectFolder + "/BasicTarget.controller";
    private const string TilePrefabPath =
        "Assets/Prefabs/UI/DungeonTile.prefab";
    private const string EffectRootName = "grpBasicTargetEffects";
    ...
    private static bool IsGenerationRequired()
    {
        if (AssetDatabase.LoadAssetAtPath<AnimationClip>(HiddenClipPath) == null ||
            AssetDatabase.LoadAssetAtPath<AnimationClip>(AimClipPath) == null ||
            AssetDatabase.LoadAssetAtPath<AnimationClip>(HoldClipPath) == null ||
        ...
        AnimationClip fireClip =
            AssetDatabase.LoadAssetAtPath<AnimationClip>(FireClipPath);
        if (fireClip == null ||
            Mathf.Abs(fireClip.length - FireAnimationDuration) > 0.001f)
        {
            return true;
        }
        ...
        return !IsAssignedObjectArray(
                   serializedTile.FindProperty("basicTargetEffectImages")) ||
               !IsAssignedObjectArray(
                   serializedTile.FindProperty("basicTargetEffectAnimators"));
    }
    ...
    private static void Generate(bool showCompletionDialog)
    {
        try
        {
            ...
            EnsureFolder("Assets/Animations/Battle", "BasicTarget");

            AnimationClip hiddenClip = GetOrCreateHiddenClip();
            AnimationClip aimClip = GetOrCreateAimClip();
            AnimationClip holdClip = GetOrCreateHoldClip();
            AnimationClip fireClip = GetOrCreateFireClip();
            ...
            if (showCompletionDialog)
            {
                EditorUtility.DisplayDialog(
                    "Basic Target Effect",
                    "Basic target animation assets and DungeonTile effect " +
                    "layers are ready. Assign the reticle Sprite to " +
                    "Basic Tower CharacterSO > Target Effect Sprite.",
                    "OK");
            }
        }
    ...
    private static AnimationClip GetOrCreateHiddenClip()
    {
        ...
        clip = CreateClip("BasicTargetHidden", 1f / 60f);
        SetConstantCurve(clip, typeof(CanvasGroup), "m_Alpha", 0f);
        SetConstantCurve(clip, typeof(RectTransform), "m_AnchorMin.y", -0.2f);
        SetConstantCurve(clip, typeof(RectTransform), "m_AnchorMax.y", 0.5f);
        SetConstantCurve(
            clip,
    ...
    private static AnimationClip GetOrCreateAimClip()
    {
        AnimationClip clip = AssetDatabase.LoadAssetAtPath<AnimationClip>(
            AimClipPath);
        if (clip != null)
        ...
        clip = CreateClip("BasicTargetAimIn", 1f);
        SetLinearCurve(clip, typeof(CanvasGroup), "m_Alpha", 0f, 1f, 1f);
        SetLinearCurve(
            clip,
            typeof(RectTransform),
            "m_AnchorMin.y",
        ...
        SetConstantCurve(
            ...
            "localEulerAnglesRaw.z",
            0f);
        AssetDatabase.CreateAsset(clip, AimClipPath);
        return clip;
    }

    private static AnimationClip GetOrCreateHoldClip()
    {
        AnimationClip clip = AssetDatabase.LoadAssetAtPath<AnimationClip>(
            HoldClipPath);
        if (clip != null)
        ...
        clip = CreateClip("BasicTargetAimHold", 1f / 60f);
        SetConstantCurve(clip, typeof(CanvasGroup), "m_Alpha", 1f);
        SetConstantCurve(clip, typeof(RectTransform), "m_AnchorMin.y", 0.15f);
        SetConstantCurve(clip, typeof(RectTransform), "m_AnchorMax.y", 0.85f);
        SetConstantCurve(
            clip,
            typeof(RectTransform),
            "localEulerAnglesRaw.z",
            0f);
        AssetDatabase.CreateAsset(clip, HoldClipPath);
        return clip;
    }

    private static AnimationClip GetOrCreateFireClip()
    {
        AnimationClip clip = AssetDatabase.LoadAssetAtPath<AnimationClip>(
            FireClipPath);
        bool isNew = clip == null;
        if (isNew)
            clip = CreateClip("BasicTargetFire", FireAnimationDuration);
        else
        {
            clip.ClearCurves();
            ConfigureClip(
        ...
        SetCurve(
            ...
            "localEulerAnglesRaw.z",
            new AnimationCurve(
                new Keyframe(0f, 0f, 562.5f, 562.5f),
                new Keyframe(
        ...
        if (isNew)
            AssetDatabase.CreateAsset(clip, FireClipPath);
        else
    ...
    private static AnimationClip CreateClip(string name, float duration)
    {
        ...
        ConfigureClip(clip, name, duration);
        return clip;
    }
        ...
        float duration)
    {
        ...
        SetCurve(
            ...
            propertyName,
            new AnimationCurve(
                new Keyframe(0f, startValue, slope, slope),
                new Keyframe(duration, endValue, slope, slope)));
    }
        ...
        AnimationClip fireClip)
    {
        ...
        AnimatorState aim = stateMachine.AddState(
            "BasicAimIn",
            new Vector3(250f, 100f));
        AnimatorState hold = stateMachine.AddState(
            "BasicAimHold",
            new Vector3(500f, 100f));
        AnimatorState fire = stateMachine.AddState(
            "BasicFire",
        ...
        ConfigureExitTransition(aim.AddTransition(hold));
        ConfigureExitTransition(fire.AddTransition(hidden));
        ...
        RuntimeAnimatorController controller)
    {
        ...
        try
        {
            ...
            RectTransform effectRoot = GetOrCreateRectTransform(
                prefabRoot.transform,
                EffectRootName,
                prefabRoot.layer);
            SetStretch(effectRoot);
            effectRoot.SetAsLastSibling();

            Image[] images = new Image[DungeonPage.MaximumPartySize];
            Animator[] animators = new Animator[DungeonPage.MaximumPartySize];
            for (int index = 0;
                 index < DungeonPage.MaximumPartySize;
                 index++)
            {
                RectTransform effect = GetOrCreateRectTransform(
                    effectRoot,
                    $"imgBasicTargetEffect_S{index + 1}",
                    prefabRoot.layer);
                effect.anchorMin = new Vector2(0.15f, -0.2f);
                effect.anchorMax = new Vector2(0.85f, 0.5f);
                effect.anchoredPosition = Vector2.zero;
                effect.sizeDelta = Vector2.zero;
                effect.localRotation = Quaternion.identity;
                effect.localScale = Vector3.one;

                Image image = GetOrAddComponent<Image>(effect.gameObject);
                image.raycastTarget = false;
                image.preserveAspect = true;

                CanvasGroup canvasGroup =
                    GetOrAddComponent<CanvasGroup>(effect.gameObject);
                canvasGroup.alpha = 0f;
                canvasGroup.blocksRaycasts = false;
                canvasGroup.interactable = false;

                Animator animator =
                    GetOrAddComponent<Animator>(effect.gameObject);
    ...
    private static bool IsAssignedObjectArray(SerializedProperty property)
    {
        if (property == null ||
            property.arraySize != DungeonPage.MaximumPartySize)
        {
            return false;
        }
...
public static class AreaExplosionEffectAssetGenerator
{
    private const float Duration = 0.5f;
    private const string MenuPath =
        "Tools/Dungeon/Effects/Create Area Explosion Effect";
    private const string EffectFolder =
        "Assets/Animations/Battle/AreaExplosion";
    private const string HiddenClipPath =
        EffectFolder + "/AreaExplosionHidden.anim";
    private const string FireClipPath =
        EffectFolder + "/AreaExplosionFire.anim";
    private const string ControllerPath =
        EffectFolder + "/AreaExplosion.controller";
    private const string AreaTowerPath =
        "Assets/Data/Characters/AreaTower.asset";
    ...
    private static bool IsGenerationRequired()
    {
        AnimationClip hidden = AssetDatabase.LoadAssetAtPath<AnimationClip>(
            HiddenClipPath);
        AnimationClip fire = AssetDatabase.LoadAssetAtPath<AnimationClip>(
            FireClipPath);
        AnimatorController controller =
            AssetDatabase.LoadAssetAtPath<AnimatorController>(ControllerPath);
        CharacterSO areaTower = AssetDatabase.LoadAssetAtPath<CharacterSO>(
            AreaTowerPath);
        if (hidden == null || fire == null || controller == null ||
            areaTower == null || Mathf.Abs(fire.length - Duration) > 0.001f)
        {
            return true;
        }

        SerializedObject serializedTower = new(areaTower);
        SerializedProperty controllerProperty =
            serializedTower.FindProperty("targetEffectController");
        return controllerProperty == null ||
    ...
    private static void Generate(bool showCompletionDialog)
    {
        try
        {
            EnsureFolder("Assets", "Animations");
            EnsureFolder("Assets/Animations", "Battle");
            EnsureFolder("Assets/Animations/Battle", "AreaExplosion");

            AnimationClip hidden = GetOrCreateHiddenClip();
            AnimationClip fire = GetOrCreateFireClip();
            AnimatorController controller = GetOrCreateController(
                hidden,
                fire);
            AssignControllerToAreaTower(controller);
            ...
            if (showCompletionDialog)
            {
                EditorUtility.DisplayDialog(
                    ...
                    "Area explosion animation is ready. Assign the explosion " +
                    "Sprite to Area Tower CharacterSO > Target Effect Sprite.",
                    "OK");
            }
        }
        catch (System.Exception exception)
    ...
    private static AnimationClip GetOrCreateHiddenClip()
    {
        ...
        clip = CreateClip("AreaExplosionHidden", 1f / 60f);
        SetConstantCurve(clip, typeof(CanvasGroup), "m_Alpha", 0f);
        SetConstantCurve(
            clip,
            typeof(RectTransform),
    ...
    private static AnimationClip GetOrCreateFireClip()
    {
        ...
        clip = CreateClip("AreaExplosionFire", Duration);
        SetCurve(
            clip,
            typeof(CanvasGroup),
            "m_Alpha",
        ...
        SetCurve(
            ...
            new AnimationCurve(
                new Keyframe(0f, 0f),
                new Keyframe(0.1f, 22.5f),
                new Keyframe(0.2f, -22.5f),
                new Keyframe(0.3f, 22.5f),
                new Keyframe(0.4f, -22.5f),
        ...
        AnimationClip fireClip)
    {
        ...
        AnimatorStateTransition transition = fire.AddTransition(hidden);
        transition.hasExitTime = true;
        transition.exitTime = 1f;
        transition.hasFixedDuration = true;
        transition.duration = 0f;
    ...
    private static void AssignControllerToAreaTower(
        RuntimeAnimatorController controller)
    {
        CharacterSO areaTower = AssetDatabase.LoadAssetAtPath<CharacterSO>(
            AreaTowerPath);
        if (areaTower == null)
            throw new System.InvalidOperationException("AreaTower asset not found.");
