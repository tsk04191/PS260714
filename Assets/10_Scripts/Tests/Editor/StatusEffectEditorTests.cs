...
public sealed class StatusEffectEditorTests
{
        ...
        params string[] expected)
    {
        FieldInfo field = typeof(StatusEffectEditorWindow).GetField(
            fieldName,
            PrivateStatic);
        if (IsSharedEffectOptionField(fieldName))
        {
            Assert.That(
                field,
                Is.Null,
                $"{fieldName} must not be duplicated by the status editor.");
            Assert.That(
                typeof(BattleAbilityEditorGUI).GetMethod(
                    "DrawEffectList",
                    BindingFlags.NonPublic | BindingFlags.Static),
                Is.Not.Null,
                "The common battle effect editor entry point is required.");
            return;
        }
    ...
    private static bool IsSharedEffectOptionField(string fieldName)
    {
        return fieldName == "EffectTypeOptions" ||
               fieldName == "EffectTargetModeOptions" ||
               fieldName == "EffectAmountModeOptions" ||
               fieldName == "StatusRemovalTargetOptions" ||
               fieldName == "StatusRemovalAmountModeOptions";
    }
