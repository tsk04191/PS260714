...
public static class ItemDefinitionCatalog
{
    ...
    private static void Register(ItemDefinitionSO definition)
    {
        ...
        if (definition is BattleItemSO battleItem &&
            battleItem.HasExecutableContent &&
            !battleItem.HasCompatibleEffects)
        {
            Debug.LogError(
                $"Battle item '{definition.name}' was excluded because " +
                "its targeting or effects are incompatible.",
                definition);
            return;
        }

        if (definition is IBattleAbilityProvider provider &&
            !AbilityDefinitionValidator.TryValidateProvider(
                provider,
                out string abilityError))
        {
            Debug.LogError(
                $"Item '{definition.name}' was excluded: {abilityError}",
                definition);
            return;
        }
