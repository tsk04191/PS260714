...
> **문서 기준일:** 2026-08-19
> 이 문서는 현재 로컬 Unity 프로젝트의 실제 플레이 콘텐츠, 공개 카드·적 카탈로그와 검증 상태를 함께 기록한 단일 게임 위키다. 코드·원본 명세·라이브 ScriptableObject 에셋의 상태가 다르면 각각을 분리해 표시한다.
...
| 메인 씬 | `ClientScene` 1개 |
| 현재 스테이지 | 연습 전투, 튜토리얼 필드, 자유 전투 |
| 지원 언어 | 한국어, 영어 |
| 모바일 화면 | Android 가로 전용, 좌·우 가로 회전 허용 |
...
1. 타이틀에서 메인 로비로 이동한다.
2. 던전 선택에서 `디버그 룸`, `자유` 또는 테마 대분류를 고른다.
3. 대분류 안에서 세부 던전을 선택하고 설명·규칙·진행도를 확인한 뒤 입장한다.
4. 현재 보유한 대원 중 무작위로 제시된 3명에서 1명을 선택한다.
5. 선택한 대원과 카드 덱으로 코어 방어 전투를 진행한다.
6. 승리 후 카드 획득 또는 코어 보호막 회복을 선택한다.
7. 다음 전투나 비전투 방으로 이동한다.
8. 마지막 전투를 이기면 클리어가 기록되고, 코어가 파괴되면 런이 종료된다.
...
`layPopup/pagLoading`은 시작 데이터 준비와 동기식 초기 생성 비용이 큰 화면 전환을 가리는 공용 입력 차단 로딩 페이지다. 시작 시 한 프레임을 먼저 표시한 뒤 데이터 준비 완료 알림에서 닫히며, 던전 선택 최초 진입, 신규 던전 런, 캐릭터·적·아이템 도감 최초 진입, 로스터·모집 최초 진입에만 적용한다. 메인·설정·단순 안내 화면과 이미 초기화된 목록 재진입에는 불필요한 로딩 화면을 표시하지 않는다.

| 화면 | 상태 | 기능 |
|---|---|---|
| 타이틀 | 실장 | 메인 진입, 공지, 설정 |
| 메인 로비 | 실장 | 대표 대원과 주요 메뉴, 출석 알림 |
| 던전 선택 | 실장 | 대분류 카드 → 세부 던전 목록·상세·진행도 → 입장 |
...
던전 선택은 2단계다. 첫 화면은 가로 카드로 대분류를 표시하고, 두 번째 화면은 왼쪽 세부 던전 목록과 오른쪽 상세 설명·규칙·진행도·입장 버튼을 표시한다. `DungeonCategorySO`가 분류 순서, 카드·배경, 로컬라이제이션과 포함 던전을 소유하므로 앞으로 `테마 1`, `테마 2` 같은 분류를 코드 분기 없이 추가할 수 있다.

현재 `디버그 룸`에는 튜토리얼 필드와 연습 전투가 들어가고, `자유`에는 `free_battle` 하나가 `무제한`으로 표시된다. 저장과 참조 호환성을 위해 내부 던전 ID `free_battle`은 변경하지 않는다. 던전 선택 배경은 전용 uGUI 블러 머티리얼로 흐림 처리하며, 던전 상세 커버의 Focus·Zoom 미리보기는 실제 `1156×340` 노출 영역과 동일한 좌표 계산을 사용한다.

던전 선택 커버는 16:9 전체화면 원본을 사용한다. 전체 배경, 카테고리 선택 카드와 세부 던전 Hero는 공통 `UiMaskedCoverImageView`의 `RectMask2D` Cover 프레이밍을 사용하며, 에셋별 초점과 확대율을 저장한다. 전체 배경은 화면 비율이 달라도 여백 없이 채우고, 선택 카드에서는 같은 원본을 카드 영역에 맞게 잘라 표시한다. 던전·카테고리 에디터에서 미리보기를 드래그하고 마우스 휠을 사용해 구도를 조정할 수 있다.
...
컨트롤 패널은 전투 상단의 `일시정지` 왼쪽에 동일한 크기로 배치된 `컨트롤` 버튼으로 열고 닫는다. 패널은 상단 전투 버튼 행과 하단 HUD를 가리지 않는 우측 안전 영역에 표시되며, 연습 전투 중에는 일반 적 생성 대기열 HUD를 숨긴다. 전체 캐릭터·적·카드 카탈로그 검색, 캐릭터 배치와 제거, 적 즉시·대기열 생성, 카드 손패 추가, 적 제거, 파티·방어선·에너지 복구와 전체 초기화를 제공한다. 연습 중 생성된 캐릭터 상태와 던전 결과는 로컬 진행 데이터에 기록하지 않는다.

디버그 표시 버튼 문구는 현재 상태가 아니라 다음 동작을 나타낸다. 표시가 꺼져 있으면 `디버그 표시 켜기`, 켜져 있으면 `디버그 표시 끄기`로 갱신하며, Scene 자동 로컬라이제이션 바인더가 이 동적 문구를 고정 키로 다시 덮어쓰지 않는다.

디버그 표시는 기본적으로 꺼져 있다. 버튼 문구는 누르면 실행할 동작을 나타내므로 꺼진 상태에서는 `디버그 표시 켜기`, 켜진 상태에서는 `디버그 표시 끄기`로 즉시 갱신된다. 켜면 실제 입력 판정과 동일한 모든 생존 유닛의 클릭 원, 캐릭터·적의 공간 점유 반경, 마지막으로 클릭한 캐릭터의 유한 능력 범위, 마지막으로 클릭한 적의 Target·Operation·Telegraph·Area 범위, 적에서 방어선 방향으로 계산되는 코어 공격 도달 선분을 전장 위에 표시한다. 일반 공격 범위를 임의의 원으로 단순화하거나 축·텍스트를 덧그리지 않으며, 기능을 끄거나 보드를 비활성화·초기화하고 대상이 제거되면 남은 표식을 즉시 비운다.

### 4.3 무제한

무제한은 여러 전투와 비전투 방을 연속으로 진행하는 메인 로그라이트 모드다. `자유`는 대분류 이름이고 `무제한`은 그 안의 현재 세부 던전 표시명이다.
...
- 적의 접근 각도는 전투 시드와 생성 순서로 결정되는 연속 무작위 값이다. 고정 섹터에 균등 배치하지 않으므로 같은 전투는 재현할 수 있으면서도 접근 모습은 자연스럽게 흩어진다.
- 각 적은 전투 시드와 생성 순서로 결정되는 외곽 원형 띠의 무작위 각도·반경에 생성되고 중앙 방어선을 향해 이동한다. 같은 시드는 같은 배치를 재현하며 기존의 고정 방사선이나 층별 목표점은 사용하지 않는다.
- 직진 경로가 다른 적에게 막힐 것으로 예상되면 적은 자신의 전방 탐색 각도 안에서 가장 작은 우회각의 빈 경로를 먼저 선택한다. 기본 탐색 범위는 전체 60도(좌우 30도)이며 `EnemySO`에서 적별로 0~180도를 설정한다. 범위 안에 안전한 경로가 없으면 앞 공간이 열릴 때까지 현재 위치에서 대기한다.
- 적의 점유 반경과 전장 분리 비율을 적용한 실제 충돌 간격으로 일부 시각적 겹침은 허용하지만 중심이 완전히 포개지는 것은 막는다. 전열이 쓰러지면 뒤의 적은 별도 진형 재배치나 순간이동 없이 다시 전진한다.
- 던전 정의의 `Maximum Active Enemies`가 동시에 전장에 존재할 수 있는 적의 전역 상한을 제공하고 개별 `BattleSO`가 필요할 때 재정의한다. 기본값은 100명이다. 외곽 띠에 충돌 없는 생성 공간이 없거나 전역 상한에 도달하면 적은 생성 대기열에 머문다.
- 전장에 표시되는 모든 적 SD 아래에는 현재 체력 비율을 나타내는 월드 체력바가 표시된다. 폭·두께·색상·위험 체력 임계치는 `DungeonHudPresentationSO`에서 조정한다.
- 적과 캐릭터 SD의 원본 좌우 방향은 각 전용 에디터의 상호 배타적인 좌·우 버튼으로 저작하며 신규 정의의 기본값은 오른쪽이다. 적은 항상 보호막 중심을 바라보고, 캐릭터는 가장 최근 이동·위치 교체·공격·스킬 등 대상이 있는 행동의 방향을 바라본다. 자기 자신 또는 대상이 없는 행동은 기존 방향을 유지한다.
...
카드용 공간 서비스는 대원과 적의 실제 월드 위치를 코어·안쪽·외곽·방어선 구역으로 판정한다. 대상 주변 적, 지정 적 뒤편의 적, 방어선에 도달한 적, 최근 5초 안에 코어를 공격한 적을 선택할 수 있다. 대원 자유 이동과 겹치던 공개 카드의 아군 이동·위치 교환 효과는 제거했다. 적을 지정 지점으로 끌어당기는 유도 장벽은 적 진군과 광역 연계에 의미가 있어 유지하며, 끌어당긴 적도 이동 경로의 충돌을 검사하여 다른 적과 완전히 겹치지 않는다. 아군 이동·교환 연산은 기존 커스텀 카드 호환을 위해 스키마에는 남아 있다.
...
| C061~C070 | 10 | 기본 공격 대기시간 완료, 다음 공격 강화·반복, 적 끌어당김, 지속 영역 |
...
대원 이동이 무료인 조작 규칙에 맞춰 위치를 옮기는 공개 카드는 방호·기본 공격 준비·다음 공격 연계로 교체했다. 현재 교체된 카드의 실제 효과는 다음과 같다.

| ID | 카드 | 현재 효과 |
|---|---|---|
| C034 | 방벽 재배치 | 대원 1명에게 보호막 35, 코어 보호막 30% 이하이면 추가 15 |
| C042 | 긴급 방호 | 대원 1명에게 보호막 20, 해로운 상태 1개 제거 |
| C061 | 진격 명령 | 대원 1명의 다음 기본 공격 피해 +10 |
| C062 | 방어 명령 | 대원 1명에게 보호막 15 |
| C063 | 즉응 명령 | 대원 1명의 기본 공격 대기시간 즉시 완료 |
| C064 | 교차 전개 | 대원 2명의 기본 공격 대기시간 즉시 완료, 다음 기본 공격 피해 +10 |
| C065 | 전원 집결 | 카드 1장 드로우, 모든 대원의 다음 기본 공격 피해 +4 |
| C066 | 일제 공세 | 모든 대원의 다음 기본 공격 피해 +12 |
| C067 | 측면 협공 | 대원 1명의 다음 기본 공격을 50% 위력으로 1회 추가 판정 |
| C070 | 완전 포위 | 모든 적에게 Opening 1스택, 모든 대원의 다음 기본 공격 피해 +10 및 대기시간 즉시 완료 |
| C086 | 전위의 방패 | 전위 대원에게 보호막 30, 대상 체력 50% 이하이면 추가 15 |

C068 봉쇄 지점의 지속 영역과 C069 유도 장벽의 적 끌어당김은 전장 위치에 의미가 있어 유지한다. 아군 이동·교환 연산은 기존 커스텀 카드 직렬화 호환을 위해 schema에는 남아 있지만 공개 100장 카탈로그에서는 사용하지 않는다.
...
| 공간 | 커스텀 카드용 아군 이동·위치 교환, 적 끌어당김, 지연·진입 영역 생성 |
| 액션 | 기본 공격 대기시간 완료, 다음 공격 피해·상태·반복 판정, 다음 액티브 비용·자원 변경, 우선 공격 대상 강제 |
...
런타임 실행기와 `DungeonPage`·`DungeonBattleTab` 연결은 구현되어 있으며 덱 선택 중 일시정지, 유효 비용 표시와 성공 시 비용 변경 소모까지 처리한다. 기본 공격 준비 연산은 공격 후딧 중인 대원을 건너뛰고 현재 공격 대기시간이 남은 대원만 0으로 만든다. 다음 틱의 기존 자동 공격 경로를 통하므로 대상 선정·공격 금지·공격 후딧 규칙을 우회하지 않는다. 다음 공격·액티브 강화는 공통 액션 실행 ID를 사용해 한 공격 안의 여러 효과가 사용 횟수를 중복 소모하지 않으며, 반복 공격은 방어 적용 전 계산 위력을 기준으로 만든다. 100장 에셋의 정적 대조는 완료했지만 캐릭터 전용 자원 대상, 트리거, 지연 영역을 포함한 실제 플레이는 Unity PlayMode 확인이 필요하다.
...
EnemySO는 전투 스탯 schema v3와 로스터 schema v1을 함께 사용한다. 공개 ID, 등급, 역할·카운터 태그, 위협도, 개별 편성 상한, 소환 비용, 전용 조우 여부, 점유 반경, 전방 탐색 각도, 코어 공격 사거리와 소수 코어 피해를 정의한다. 자동 전투의 체력은 기존 기준 체력 20에 대한 각 로스터 정의의 상대 배율을 난이도 체력에 투영한다.
...
- 월드 반경 기반 대상, 피해 연결·반사, 대상 우선순위와 조건 필터
...
현재 46종 로스터에는 플레이어 캐릭터의 체력을 능동적으로 직접 공격하는 적 능력이 없다. 플레이어 캐릭터를 명시적으로 대상으로 삼는 능력은 G024의 에너지 회복 감소, G028의 자동 행동 주기 증가, S002의 누적 피해량 최고 대원 기절이며 모두 체력 피해가 아니다. 캐릭터에게 적이 주는 실제 피해는 G025 반사 잔재가 8초마다 준비하는 다음 직접 피해 35% 반사가 유일하다. 반사 피해는 공격한 대원에게 돌아가며 대원 보호막이 먼저 흡수한다.

따라서 대원 회복·보호막 카드는 현재 G025 반사, C039·C091 같은 체력 비용, 전투 승리 후 런 체력 감소와 이전 전투에서 이어진 손상을 복구하는 용도가 중심이다. 적의 직접·범위·지속 피해를 상시 대응하는 생존 수단으로는 아직 활용되지 않는다. 공통 적 능력 실행기는 캐릭터 직접 피해를 지원하고 회귀 테스트도 있지만 실제 EnemySO 로스터에는 해당 피해 능력이 없다.

원형 전장은 적을 외곽 띠의 충돌 없는 무작위 위치에 생성하고 각 적이 중앙으로 독립 이동하게 한다. 앞 적에게 막힌 적은 기본 전방 60도 안의 공간을 미리 탐색해 가장 가까운 안전 방향으로 흐르고, 공간이 없으면 대기하여 RTS 군중처럼 자연스럽게 밀집한다. 탐색 각도는 적별로 다르게 저작할 수 있다. 고정 층과 접촉 깊이는 배치 권위에서 제거했으며, 월드 범위 능력은 실제 거리로 판정한다. 기본 최대 활성 적은 100명이고 던전에서 정하며 개별 고정 전투가 재정의할 수 있다. 코어 공격 사거리는 실제 월드 배치에 적용되어 근접 적은 방어선에 닿아야 하고, 충분한 사거리를 가진 적은 뒤에서도 코어를 공격한다.
...
Android 빌드는 자동 회전 중 세로와 역세로를 비활성화하고 Landscape Left·Right만 허용한다. 기기를 반대로 돌리면 좌·우 가로 방향끼리는 전환되지만 세로 화면으로는 전환되지 않는다. `ProjectSettings`가 저장소 추적 대상이 아닌 구조를 보완하기 위해 `AndroidLandscapeBuildGuard`가 Android 빌드 전 같은 설정을 다시 적용하고 검증한다. 기존에 생성된 APK는 변경되지 않으므로 새 APK 빌드·설치 후 실제 기기 회전 확인이 필요하다.
...
| 상태 효과 | 15 | 실장 |
| 전투 VFX 큐 | 9 | 실장 |
| 던전 정의 | 3 | 튜토리얼 필드·연습 전투·무제한(`free_battle`) |
| 던전 대분류 | 2 | 디버그 룸·자유, 테마 분류 확장 가능 |
| 튜토리얼 | 1 | 튜토리얼 필드 연결 |
| 던전 이벤트 | 2 | 미연결, 효과 없음 |
| 휴식 | 1 | 자유 전투 연결 |
| 던전 흐름 정책 | 2 | 고정·교차 흐름 |
...
| 던전 상점 | 0 | 코드만 존재 |
| 로컬라이제이션 문자열 | CSV 696 / 런타임 696 | 한국어·영어 생성 테이블 갱신 완료 |
...
| P1 | 캐릭터를 능동 공격하는 적 부재 | G025 반사 외에는 캐릭터 체력 피해가 없어 대원 회복·보호막 카드의 적 대응 가치가 제한됨 |
...
| P2 | 적 군중 이동 PlayMode 확인 필요 | 무작위 외곽 생성·충돌·사거리 EditMode 회귀 코드는 작성했지만 실제 2.5D 카메라에서 밀집 정도와 우회 연출을 확인해야 함 |
| P2 | 최신 EditMode 전체 재실행 필요 | 2026-08-19 결과는 773개 중 769개 통과였고 실패 4개의 테스트 설정을 수정했지만 전체 재실행 결과는 아직 없음 |
| P2 | Android 가로 전용 APK 실기기 확인 필요 | 프로젝트 설정과 빌드 가드는 반영했지만 새 APK 매니페스트와 기기 회전은 아직 재검증하지 않음 |
| P2 | PlayMode 통합 테스트 부족 | 전체 플레이 흐름 회귀 위험이 큼 |

### 권장 다음 마일스톤

1. 2026-08-19 실패 4건과 전체 EditMode 테스트를 다시 실행하고, 수정된 수련 PlayMode 픽스처도 함께 확인한다.
2. 새 Android APK를 빌드·설치해 양쪽 가로 회전은 허용되고 세로·역세로는 차단되는지 확인한다.
3. 실제 2.5D 전장에서 12명·36명·100명 배치를 만들어 군중 간격, 큰 적의 부분 겹침, 체력바 가독성, 전열 사망 후 전진과 원거리 적의 후방 공격을 확인한다.
4. 캐릭터 회복·보호막을 적 대응 생존 수단으로 유지할 경우 직접·범위·지속 피해 적을 어느 등급에 배치할지 결정하고 로스터에 연결한다.
5. G022 능력 이력 재현, 실제 적 월드 지대, 파괴 가능한 알과 E001 보호막 층을 전용 런타임 상태·프레젠테이션으로 완성한다.
6. B001의 세 페이즈, 소환 상한, 충전 중단과 적 전조 VFX를 PlayMode 통합 테스트로 고정한다.
7. 100개 카드의 도감 표시·덱 로딩과 시작 카드 10종, 보상 카드, 비용 4~5 해금 및 모든 schema v2 연산군을 PlayMode에서 검증한다.
8. 한국어·영어 카드·적 도감에서 이름·설명·줄바꿈과 변수 표시를 확인한다.
9. 신규 대원 획득 경로를 연결해 복수 대원·구역·역할 조건 카드를 실제 런에서 사용할 수 있게 한다.
10. 효과 있는 이벤트 1개와 상품 있는 상점 1개를 자유 전투에 연결하고, 5전 고정 검증 런 전체 경로를 PlayMode 테스트로 고정한다.
...
- `DungeonBoardView`의 원형 군중 이동은 시드 기반 외곽 생성 위치, 적별 점유 반경, 전방 공간 탐색과 코어 사거리 판정을 함께 관리한다. 이동 시 주변 공간 인덱스로 가까운 충돌 후보만 확인하며, 적별 전방 부채꼴 안에 경로가 없을 때만 대기한다.
...
- `AndroidLandscapeBuildGuard`가 Android 빌드 전 세로 방향을 끄고 양쪽 가로 방향만 허용하도록 Player Settings를 적용한다.
...
- 프로젝트 소유 커스텀 에디터의 에셋 목차와 선택 목록은 `PS260714EditorAssetDisplayName`을 공통 사용한다. 표시 이름은 현재 언어에서 조회된 `nameLocalizationKey` 또는 `titleLocalizationKey` 값, 단일 fallback 이름, 에셋 파일명 순서로 결정하며 앞 순위가 비어 있거나 조회되지 않을 때만 다음 순위를 사용한다.
- 공통 fallback 검색 대상은 `fallbackName`, `fallbackTitle`, `displayName`, `characterName`이며 언어별 fallback 필드를 에셋에 추가하지 않는다.
- 적 에디터 목록 상단에는 크기가 고정된 드롭다운 두 개가 있다. 등급 필터는 전체·일반·특수·엘리트·보스를 선택하고, 정렬은 이름순 또는 등급순을 선택한다. 등급순에서도 같은 등급 안에서는 표시 이름과 파일명 순으로 정렬한다.
- 캐릭터·적 에디터의 SD 원본 방향은 좌·우 중 하나만 활성화되는 공통 버튼으로 입력한다. 캐릭터는 최근 이동·행동 대상, 적은 코어 중심을 향하도록 런타임에서 좌우 반전을 계산한다.

### 검증 상태

- 2026-08-19 `AndroidLandscapeBuildGuard`까지 포함한 `dotnet build PS260714.slnx`는 오류 0개로 성공했다. 표시된 경고 30개는 UniTask, Input System, Searcher, Timeline, Visual Scripting 패키지 코드이며 새 프로젝트 코드 경고는 없다.
...
- 최신 제공 EditMode Test Runner 결과는 773개 중 769개 통과, 4개 실패다. 실패는 `ReadyBasicAttack` 테스트의 아군 대상 설정 2건, 미저장 씬과 충돌한 로딩 페이지 설치 테스트 1건, 연습 전투 디버그 이벤트 기대 횟수 1건이었다. 네 테스트를 현재 계약에 맞게 수정했지만 수정 후 전체 Test Runner 재실행 결과는 아직 없다.
- 이전 PlayMode 1건은 테스트용 `CharacterRuntime`의 필수 HUD 참조와 `DungeonBoardView`의 `BattleVfxPlayer` 누락으로 초기화 단계에서 실패했다. 실제 `CharacterInfo` 프리팹과 VFX 컴포넌트를 사용하도록 픽스처를 수정했으며 최신 PlayMode 재실행 결과는 아직 없다.
- 적 전투 스키마 v3와 기존 적의 점유 반경·전방 탐색 각도·코어 사거리 마이그레이션을 검증한다. 편대 기본값·용량·최소 간격·전열 압축·근접/원거리 공격·끌어당김·공간 선택·레거시 격자 및 전방 공간 탐색 회귀 테스트를 함께 유지한다.
...
| 공통 에디터 표시·SD 방향 | `Assets/10_Scripts/Editor/EditorUtil.cs` |
| Android 가로 빌드 가드 | `Assets/10_Scripts/Editor/AndroidLandscapeBuildGuard.cs` |
